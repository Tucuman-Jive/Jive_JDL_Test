...
public class ChannelRepositoryWithBagRelationshipsImpl implements ChannelRepositoryWithBagRelationships {
    ...
    public Optional<Channel> fetchBagRelationships(Optional<Channel> channel) {
        return channel.map(this::fetchMembers);
    }
    ...
    public List<Channel> fetchBagRelationships(List<Channel> channels) {
        return Optional.of(channels).map(this::fetchMembers).orElse(Collections.emptyList());
    }

    Channel fetchMembers(Channel result) {
        return entityManager
            .createQuery("select channel from Channel channel left join fetch channel.members where channel is :channel", Channel.class)
            .setParameter("channel", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Channel> fetchMembers(List<Channel> channels) {
        ...
        List<Channel> result = entityManager
            .createQuery(
                "select distinct channel from Channel channel left join fetch channel.members where channel in :channels",
                Channel.class
            )
