...
public class DirectMessageRepositoryWithBagRelationshipsImpl implements DirectMessageRepositoryWithBagRelationships {
    ...
    public Optional<DirectMessage> fetchBagRelationships(Optional<DirectMessage> directMessage) {
        return directMessage.map(this::fetchUserProfiles);
    }
    ...
    public List<DirectMessage> fetchBagRelationships(List<DirectMessage> directMessages) {
        return Optional.of(directMessages).map(this::fetchUserProfiles).orElse(Collections.emptyList());
    }

    DirectMessage fetchUserProfiles(DirectMessage result) {
        return entityManager
            .createQuery(
                "select directMessage from DirectMessage directMessage left join fetch directMessage.userProfiles where directMessage is :directMessage",
                DirectMessage.class
            )
    ...
    List<DirectMessage> fetchUserProfiles(List<DirectMessage> directMessages) {
        ...
        List<DirectMessage> result = entityManager
            .createQuery(
                "select distinct directMessage from DirectMessage directMessage left join fetch directMessage.userProfiles where directMessage in :directMessages",
                DirectMessage.class
            )
